...
contract TaxPayerCompany is ITaxPayerCompany, ReentrancyGuard {
    ...
    function changeCompanyAdmin(uint256 _companyID, address _newAdmin) public onlyCompanyAdmin(_companyID) {
          require(_newAdmin != address(0), "CANNOT BE ZERO ADDRESS");
          require(_companyID <= numberOfCompanies, "NOT A VALID COMPANY ID");

          address previousAdmin = companies[_companyID].admin;
          companies[_companyID].admin = _newAdmin;
    ...
    function addProposal(uint256 _proposalID, uint256 _companyID) public {
        (companies[_companyID].proposals).push(_proposalID);
    }
